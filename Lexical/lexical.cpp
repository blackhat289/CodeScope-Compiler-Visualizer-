...
auto it = optrs.begin();
for(; it != optrs.end() && it->first != str; ++it);
...
auto it = optrs.begin();
for(; it != optrs.end() && it->first != str; ++it);
if(it != optrs.end()) return it->second;
return ERROR;
}

bool Lexical::isId(const string &str) {
if(!isalpha(str[0]) && str[0] != '*') return false;
for(auto c: str)
...
auto it = find(indetifiers.begin(), indetifiers.end(), make_pair(str, ID));
...
auto it = find(constants.begin(), constants.end(), make_pair(str, NUMBER));
if(it != constants.end())
...
auto it = find(strings.begin(), strings.end(), make_pair(str, STRING));
if(it != strings.end())
...
bool Lexical::isChar(const string &chr) {
	return chr.length() == 1 || (chr[0] == '\\' && chr.length() == 2);
}

int Lexical::getCharPointer(const string &chr) {
auto it = find(chars.begin(), chars.end(), make_pair(chr[0], CHAR));
if(it != chars.end())
return it - chars.begin();
chars.push_back(make_pair(chr[0], CHAR));
return chars.size() - 1;
}

bool Lexical::getIn() {
++row;
return (bool)getline(cin, in);
}
...
char c = in[column];
	...
	if(isalpha(c)) {
		...
		string s = cut(column+2, in.length());
